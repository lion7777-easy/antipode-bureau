...
triggerGiftForLocation(cached.lat, cached.lng, cached.cityData);
   ...
   isSearching = true;
    ...
    let anti = null;  // 声明在外部
    try {
        anti = calculateAntipode(amapResult.lat, amapResult.lng);
    ...
    const tempCityData = {
        ...
        antipode_lat: anti ? anti.lat : 0,   // 安全访问
        antipode_lng: anti ? anti.lng : 0,   // 安全访问
    ...
    let anti = null;  // 声明在外部
    try {
        anti = calculateAntipode(tianDiTuResult.lat, tianDiTuResult.lng);
        const geoRes = await fetch(`/api/reverse-geocode?lng=${anti.lng}&lat=${anti.lat}`, {
            headers: { 'ngrok-skip-browser-warning': 'true' }
        });
        const geoData = await geoRes.json();
    ...
    const tempCityData = {
        ...
        antipode_name: antipodeName,
        antipode_name_en: antipodeNameEn,
        antipode_lat: anti ? anti.lat : 0,   // 新增
        antipode_lng: anti ? anti.lng : 0,   // 新增
        poem: '「这一刻，你与地球背面，同频呼吸。」',
        poem_en: 'In this moment, you breathe in sync with the far side of the earth.',
        origin_image: '',
        antipode_image: ''
...
if (coord) {
    ...
    antipode_name: antipodeName,
    antipode_name_en: antipodeNameEn,
    antipode_lat: antipode.lat,      // 新增
    antipode_lng: antipode.lng,      // 新增
...
await drawStampWithMap(ctx, x2, y, imgWidth,
    ...
    // ---- 获取对跖点显示名称 ----
    let antipodeDisplayName = city.antipode_name || '地球另一端';
    let antipodeDisplayNameEn = city.antipode_name_en || '';
    ...
    if (typeof getAntipodeProvince === 'function') {
        try {
            ...
            if (antiLat && antiLng) {
                ...
                if (result && result.level === 'province') {
                    antipodeDisplayName = result.displayName;      // 阿根廷布宜诺斯艾利斯
                    antipodeDisplayNameEn = result.displayNameEn;  // Buenos Aires
                } else if (result && result.level === 'country') {
                    antipodeDisplayName = result.displayName;      // 法国
                    antipodeDisplayNameEn = result.displayNameEn;  // France
                } else if (result && result.level === 'ocean') {
                    antipodeDisplayName = result.displayName;      // 太平洋
                    antipodeDisplayNameEn = result.displayNameEn;  // Pacific Ocean
                }
            }
        } catch (e) {
            console.warn('对跖点匹配失败，使用数据库值:', e);
        }
    }
    ...
    ctx.fillText(antipodeDisplayName, x2 + imgWidth / 2, titleY);

    ctx.fillStyle = textColor;
    ctx.globalAlpha = 0.6;
    ctx.font = '400 16px "Noto Serif SC", "Noto Sans SC", serif';
    ctx.fillText(`(${cityNameEn})`, x1 + imgWidth / 2, titleY + 26);
    ctx.fillText(antipodeDisplayNameEn ? `(${antipodeDisplayNameEn})` : '', x2 + imgWidth / 2, titleY + 26);
    ctx.globalAlpha = 1.0;
