...
const AMAP_KEY = 'd6846dea147b497922afd3f9b121b429'; // 地图使用
const AMAP_WEATHER_KEY = '7d290d0879a55c759b7de4d661281bed'; // 天气使用
...
// ---- 获取天气（中国用高德，国外用 Open-Meteo） ----
async function fetchWeather(lat, lng) {
    // 国内城市：直接走高德，完全不使用缓存
    if (isInChina(lat, lng)) {
        try {
            // 逆地理编码获取 adcode
            const regeoUrl = `https://restapi.amap.com/v3/geocode/regeo?location=${lng},${lat}&key=${AMAP_WEATHER_KEY}&extensions=base`;
            const regeoResp = await fetch(regeoUrl);
            const regeoData = await regeoResp.json();
            if (regeoData.status !== '1' || !regeoData.regeocode) {
                throw new Error('逆地理编码失败');
            }
            const adcode = regeoData.regeocode.addressComponent.adcode;
            if (!adcode) throw new Error('未获取到 adcode');

            // 调用高德天气 API
            const weatherUrl = `https://restapi.amap.com/v3/weather/weatherInfo?city=${adcode}&key=${AMAP_WEATHER_KEY}&extensions=base`;
            ...
            const weatherObj = {
                ...
                weathercode: amapWeatherMap[live.weather] || 99,
                weatherText: live.weather,
            };
            // ✅ 直接返回，不存入缓存
            return weatherObj;
        } catch (e) {
            console.warn('⚠️ 高德天气失败，降级到 Open-Meteo:', e.message);
            // 高德失败，继续往下走（不检查缓存）
        }
    }
    ...
    try {
        ...
        if (!response.ok) throw new Error('Open-Meteo 请求失败');
        const data = await response.json();
        const weather = data.current_weather;
        setCachedWeather(lat, lng, weather);
        return weather;
    } catch (e) {
        console.warn('❌ Open-Meteo 也失败:', e.message);
        return null;
    }
}
// ===== 高德天气文本 → Open-Meteo 代码映射（用于兼容现有图标体系） =====
...
if (searchCache.has(cacheKey)) {
    console.log('✅ 命中搜索缓存（位置），天气将实时刷新');
    const cached = searchCache.get(cacheKey);
    const lat = cached.lat;
    const lng = cached.lng;
    const cityData = cached.cityData;
    const cityName = cached.cityName;

    // --- 1. 重新获取天气（不走缓存），捕获异常 ---
    let originWeather, antipodeWeather, antiLat, antiLng;
    try {
        const weather = await fetchBothWeather(lat, lng);
        originWeather = weather.originWeather;
        antipodeWeather = weather.antipodeWeather;
        ...
        console.warn('⚠️ 缓存分支获取天气失败:', e.message);
        // 天气获取失败，重置按钮并退出
        isSearching = false;
        const btn = document.getElementById('searchBtn');
        if (btn) {
    ...
    // --- 3. 重新生成诗句 ---
    const isDatabaseCity = cityData && cityData.id && cityData.poem;
    let poemCN, poemEN;
    if (isDatabaseCity) {
        const result = applyWeatherPoem(cityData, originWeather, antipodeWeather);
        poemCN = result.poemCN;
        poemEN = result.poemEN;
    } else {
        const result = getPoemForNonDbCity(cityName, lat, lng, originWeather, antipodeWeather);
        poemCN = result.cn;
        poemEN = result.en;
    }
    ...
    // --- 6. 更新缓存中的天气数据（可选） ---
    cached.originWeather = originWeather;
    cached.antipodeWeather = antipodeWeather;

      // ✅ 重置搜索状态
    console.log('🔍 缓存分支执行完毕，准备重置按钮');
    isSearching = false;
    const btn = document.getElementById('searchBtn');
    if (btn) {
        btn.disabled = false;
...
// ===== 排行榜 =====
async function loadRanking(period = 'week') {
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            list.innerHTML = '<li style="text-align:center;color:rgba(245,240,232,0.3);padding:20px;">暂无数据</li>';
        }
    } catch(e) {
